...
export default function ProcessingQueue({ onGoToUpload }: { onGoToUpload?: () => void }) {
  const { t } = useTranslation()
      ...
      <div className="flex items-center justify-end gap-2">
        {isUploading && (
          <Button
            variant="destructive"
            size="sm"
            onClick={() => { cancelUpload(); toast(t("Cancelled remaining uploads")) }}
        ...
        {!isUploading && jobs.length > 0 && (
          <Button
            variant="outline"
            size="sm"
            onClick={() => { clearJobs(); toast(t("History cleared")) }}
            className="font-semibold shadow-sm"
          >
        ...
        <div className="rounded-xl border bg-card shadow-sm">
          <div className="flex flex-col items-center justify-center py-24 px-6 text-center space-y-4">
            ...
            <p className="text-sm text-muted-foreground max-w-[300px] leading-relaxed">
              {t("Attach files to your books and start uploading.")}
            </p>
            {onGoToUpload && (
              <Button variant="secondary" className="mt-4 font-semibold" onClick={onGoToUpload}>
