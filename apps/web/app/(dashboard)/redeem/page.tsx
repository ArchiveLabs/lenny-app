...
import { AlertCircle, CheckCircle2, Clock, Download, Loader2, Ticket, XCircle } from "lucide-react"
...
export default function RedeemPage() {
  ...
  const { data } = useQuery({
    ...
    refetchInterval: (query) =>
      query.state.data?.imports.some((job) => IN_FLIGHT.includes(job.status)) ? 2000 : false,
  })
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-500 space-y-10 p-2 md:p-6 lg:p-8">
      <div className="flex flex-col space-y-2">
        <h2 className="text-3xl font-bold tracking-tight">{t("Redeem a Bundle")}</h2>
      ...
      <form onSubmit={handleSubmit} className="rounded-xl border bg-card shadow-sm p-6 space-y-4 max-w-xl">
        ...
        {result && (
          <div className="flex items-start gap-2 rounded-lg border border-green-200 bg-green-50 dark:bg-green-950/20 dark:border-green-900/40 px-4 py-2.5 text-sm text-green-700 dark:text-green-400">
            <CheckCircle2 className="w-4 h-4 mt-0.5 shrink-0" />
            <span>
              {t("Redeemed {{count}} book(s). Downloading them now — watch the table below.", { count: result.count })}
            </span>
          </div>
        )}
      </form>
            ...
            <Table className="table-fixed">
              ...
              <TableHeader className="bg-muted/50 border-b">
                <TableRow className="hover:bg-transparent cursor-default">
                  ...
                  <TableHead className="hidden md:table-cell font-semibold text-muted-foreground text-xs uppercase tracking-wider">{t("Detail")}</TableHead>
                ...
                {imports.map((job) => (
                  <TableRow key={`${job.source}:${job.olid}`} className="transition-colors duration-200">
                    ...
                    <TableCell className="hidden md:table-cell whitespace-normal">
                      <span className="text-[11px] text-muted-foreground">{job.error || "—"}</span>
                    </TableCell>
