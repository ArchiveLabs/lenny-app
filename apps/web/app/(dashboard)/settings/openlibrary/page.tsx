...
import { OLStatus, OLStatusSchema } from "@/types/api"
...
export default function OpenLibrarySettingsPage() {
  ...
  const statusQuery = useQuery({
    ...
    queryFn: async () => {
      const res = await fetchAdmin("ol/status")
      if (!res.ok) {
        const d = await res.json().catch(() => ({}))
        throw new Error(
          res.status === 401 || res.status === 403
            ? t("Unauthorized: please log out and back in.")
            : d.error || t("Failed to fetch status")
        )
      }
      return handleApiResponse<OLStatus>(res, OLStatusSchema)
    },
  })

  const loginMutation = useMutation({
    mutationFn: async ({ replace = false }: { replace?: boolean }) => {
      const res = await fetchAdmin("ol/login", {
        method: "POST",
    ...
    onSuccess: (data) => {
      toast.success(data.message || t("Connected successfully"))
      setEmail("")
      setPassword("")
      setFormError(null)
    ...
    onError: (error: any) => {
      if (error.status === 409) {
        ...
        setFormError(t("Too many attempts — please wait a few minutes before trying again."))
      } else if (error.status === 401) {
        setFormError(t("Incorrect email or password. Please check your Open Library or Internet Archive credentials."))
      } else {
        setFormError(error.data?.message || t("Something went wrong. Please try again."))
      }
    },
  })
  ...
  const logoutMutation = useMutation({
    mutationFn: async () => {
      const res = await fetchAdmin("ol/logout", { method: "POST" })
      if (!res.ok) throw new Error(t("Logout failed"))
      return res.json()
    },
    onSuccess: (data) => {
      toast.success(data.message || t("Disconnected"))
      queryClient.invalidateQueries({ queryKey: ["admin_ol_status"] })
    },
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">{t("OpenLibrary")}</h1>
          <p className="text-sm text-muted-foreground mt-1">
            {t("Authenticate with an OpenLibrary account to enable lending on this Lenny instance.")}
          </p>
        </div>
      ...
      {statusQuery.isError && (
        <div className="flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 dark:bg-red-950/30 dark:border-red-900/50 px-4 py-3 text-sm">
          ...
          <div>
            <p className="font-medium text-red-800 dark:text-red-300">{t("Cannot reach backend")}</p>
            <p className="text-red-600 dark:text-red-400 text-xs mt-0.5">
              {statusQuery.error instanceof Error ? statusQuery.error.message : t("Unknown error")}
            </p>
          </div>
        </div>
      )}
      ...
      {isConnected && (
        <div className="flex items-center justify-between rounded-lg border bg-muted/20 px-4 py-3.5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium">
                <Trans
                  i18nKey="Signed in as <span>{{username}}</span>"
                  values={{ username: connectedUser }}
                  components={{ span: <span className="text-muted-foreground font-normal" /> }}
                />
              </p>
              <p className="text-xs text-muted-foreground mt-0.5">{t("Lending is active on this instance")}</p>
            </div>
          </div>
          <Button
            variant="outline"
          ...
          >
            ...
            {logoutMutation.isPending ? t("Disconnecting…") : t("Disconnect")}
          </Button>
        </div>
      )}
      ...
      {!statusQuery.isLoading && !isConnected && (
        <>
          <section className="space-y-4">
            <div>
              <h2 className="text-sm font-medium">{t("Sign in to Open Library Account")}</h2>
              <p className="text-xs text-muted-foreground mt-0.5">
                {t("Use your openlibrary.org credentials.")}
              </p>
            </div>
            ...
            >
              <div className="space-y-1.5">
                <Label htmlFor="ol-email">{t("Email")}</Label>
                <Input
                  id="ol-email"
                  type="email"
              ...
              <div className="space-y-1.5">
                <Label htmlFor="ol-password">{t("Password")}</Label>
              ...
              >
                ...
                {loginMutation.isPending ? t("Connecting…") : t("Connect account")}
              </Button>
            </form>
          </section>
        </>
      ...
      <AlertDialog open={disconnectDialogOpen} onOpenChange={setDisconnectDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("Disconnect OpenLibrary?")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("This removes the OpenLibrary S3 keys from the server. Book lending will stop working until you reconnect with OpenLibrary.")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={replaceDialogOpen} onOpenChange={setReplaceDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("Already connected")}</AlertDialogTitle>
            <AlertDialogDescription>
              <Trans
                i18nKey="Lenny is already signed in as <strong>{{username}}</strong>. Replace with the new account?"
                values={{ username: conflictUsername }}
